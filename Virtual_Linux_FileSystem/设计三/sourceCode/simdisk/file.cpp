...
void cmd_cat(const char* fileName) 
{
	...
	if (readInode(tempNGroup, tempNInode, true) == false)
		return;
	//打开文件
	cout << "正在打开文件......\n";
	shellOutput("正在打开文件......\n");
	...
	else
	{
		...
		{
			readInode(tempNGroup, tempNInode, false);  //先解除读
			if (writeInode(tempNGroup, tempNInode, true) == false)  //查看是否可写
				return;
				...
				&& presentUser.type == VISITOR)
			{
				cout << "权限不足！\n";
				shellOutput("权限不足！\n");
				writeInode(tempNGroup, tempNInode, false);
				return;
			}
			cout << "输入新内容：\n";
			shellOutput("输入新内容：\n");
			...
			saveDisk();
			writeInode(tempNGroup, tempNInode, false);  //解除写
		}
		else;
	}
	readInode(tempNGroup, tempNInode, false);  //解除读
}
